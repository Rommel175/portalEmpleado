...
export default function EquipoAdmin({ equipo }: { equipo: Equipo[] }) {
    ...
    useEffect(() => {
        ...
        const jornadaRealTime = supabase
            .channel('realtime-fichaje_jornada-admin')
            .on('postgres_changes', {
                event: '*',
                schema: 'public',
        ...
        const eventosRealTime = supabase
            .channel('realtime-fichaje_eventos-admin')
            .on('postgres_changes', {
                event: '*',
                schema: 'public',
        ...
        <div className={styles.table}>
...

            {
                users.map((item, index) => {
                    return <EquipoItem key={index} id={item.id} nombre={item.nombre} apellido={item.apellido} email={item.email} estado={item.estado} foto={item.image} localizacion={(currentDate == item.fecha) ? item.localizacion : '-'} inicio={(currentDate == item.fecha) ? item.hora : '-'} final={(currentDate == item.fecha) ? item.hora_aprox_salida : '-'} />
                })
            }
        </div>
    );
