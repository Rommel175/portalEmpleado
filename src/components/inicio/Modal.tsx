        ...
        if (!data || data.length == 0) {
            ...
            if (dataFichaje && dataFichaje.length > 0) {
                ...
                const { error: errorInsertFichajeEvent } = await supabase
                    ...
                    .insert({ fichaje_id: idFichaje, evento: 'Inicio Jornada', date: date.toISOString(), localizacion: localizacionFichaje });
            ...
            const { error: errorInsertFichajeEvent } = await supabase
                .from('fichaje_eventos')
                .insert({ fichaje_id: fichajeId, evento: 'Inicio Jornada', hora: date.toISOString(), localizacion: localizacionFichaje });
    ...
    return (
        ...
        /*isOpen ? (
            !profile[0].alta ? (
                <div className={styles.overlay}>
                    <div className={styles.modalContainer}>
                        <div className={styles.form}>
                            ...
                            <div className={styles.formGroup}>
                                <h2>Al parecer no estas dado de alta en nuestra plataforma, consulta a un administrador</h2>
                                <div className={styles.buttons}>
                ...
                ( (!fichaje || fichaje.length == 0) && (profile[0].estado === 'Inactivo' || profile[0].estado === 'Jornada Finalizada')) && (
                    <div className={styles.overlay}>
