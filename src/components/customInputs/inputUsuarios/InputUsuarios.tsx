...
export default function InputUsuarios({checkedState, setCheckedState, totalUsuarios}: Props) {
    ...
    return (
        <div className={styles.filtro} onClick={handleDropdown} ref={dropdownRef}>
            ...
            <p>Usuarios</p>
            <p className={`${(totalUsuarios > 0) ? styles.totalUsers : styles.hidden}`}>{totalUsuarios}</p>
                    ...
                    <div className={styles.container}>
                        {
                            profiles.map((item) => {
                                return <label className={styles.option} key={item.id} onClick={handleLabelClick}>
                                    <SelectOne checkedState={!!checkedState[item.id]} setCheckedState={setCheckedState} id={item.id} />
                                    <p>{item.name}</p>
                                </label>
                            })
                        }
