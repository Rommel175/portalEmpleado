...
import styles from './dropdownExportarInformes.module.css';
...
export default function DropdownExportarInformes({ usersData, checkedState, titulo }: { usersData: UserData[], checkedState: { [key: string]: boolean }, titulo: string }) {
    ...
    function handleExportExcel() {
        const exportar = async () => {
            ...
            worksheet.columns = [
                ...
                { header: titulo, key: 'horas_trabajadas', width: 30 },
            ...
            if (selectedProfiles.length > 0) {
                ...
                const data = showProfiles.map(item => [
                    ...
                    item.email?.toString() ?? '',
                    item.horas_trabajadas?.toString() ?? '',
    ...
    function handleExportPdf() {
        const exportar = async () => {
            ...
            const doc = new jsPDF();
            const headers = [['ID', 'Nombre', 'Apellido', 'Email', titulo, 'Horas restantes']];
